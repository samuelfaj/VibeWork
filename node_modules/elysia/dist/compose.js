...
const allocateIf = (value, condition) => condition ? value : "", overrideUnsafeQuote = (value) => (
  // '`' + value + '`'
  "`" + value.replace(/`/g, "\\`").replace(/\${/g, "$\\{") + "`"
), overrideUnsafeQuoteArrayValue = (value) => value.replace(/`/g, "\\`").replace(/\${/g, "$\\{"), defaultParsers = [
  "json",
  "text",
  "urlencoded",
  "arrayBuffer",
...
if(_setCookie){`, cookieMeta.sign === !0)
        _encodeCookie += `for(const [key, cookie] of Object.entries(_setCookie)){c.set.cookie[key].value=await signCookie(cookie.value,${secret ? overrideUnsafeQuote(secret) : "undefined"})}`;
      else {
        typeof cookieMeta.sign == "string" && (cookieMeta.sign = [cookieMeta.sign]);
        for (const name of cookieMeta.sign)
          _encodeCookie += `if(_setCookie[${overrideUnsafeQuote(name)}]?.value)c.set.cookie[${overrideUnsafeQuote(name)}].value=await signCookie(_setCookie[${overrideUnsafeQuote(name)}].value,${secret ? overrideUnsafeQuote(secret) : "undefined"})
`;
      }
      _encodeCookie += `}
`;
...
`);
  ...
  if (fnLiteral += "try{", hasCookie) {
    const get = (name, defaultValue) => {
      ...
      return value === void 0 ? "" : value ? typeof value == "string" ? `${name}:${overrideUnsafeQuote(value)},` : value instanceof Date ? `${name}: new Date(${value.getTime()}),` : `${name}:${value},` : typeof defaultValue == "string" ? `${name}:"${defaultValue}",` : `${name}:${defaultValue},`;
    }, options = cookieMeta ? `{secrets:${cookieMeta.secrets !== void 0 ? typeof cookieMeta.secrets == "string" ? overrideUnsafeQuote(cookieMeta.secrets) : "[" + cookieMeta.secrets.map(overrideUnsafeQuoteArrayValue).reduce((a, b) => a + `'${b}',`, "") + "]" : "undefined"},sign:${cookieMeta.sign === !0 ? !0 : cookieMeta.sign !== void 0 ? typeof cookieMeta.sign == "string" ? overrideUnsafeQuote(cookieMeta.sign) : "[" + cookieMeta.sign.map(overrideUnsafeQuoteArrayValue).reduce((a, b) => a + `'${b}',`, "") + "]" : "undefined"},` + get("domain") + get("expires") + get("httpOnly") + get("maxAge") + get("path", "/") + get("priority") + get("sameSite") + get("secure") + "}" : "undefined";
...
` + fnLiteral
      ...
      Cookie: hasCookie ? import_index.Cookie : void 0,
...
` + fnLiteral
    ...
    TransformDecodeError: import_value.TransformDecodeError,
